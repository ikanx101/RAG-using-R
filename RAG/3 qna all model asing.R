...
model_selection = c("swardiantara/sentence-problem-embedding",
                    "sentence-transformers/all-MiniLM-L6-v2")

model <- transformers$SentenceTransformer(model_selection[1])
...
reader     <- read_trans$pipeline(task      = "question-answering",
                                  model     = "derhan/IndoBert-QA",
                                  tokenizer = "derhan/IndoBert-QA")
...
mulai_donk = function(){
  ...
  vec_a         = ques_matrix[,1]
  batas_matriks = dim(embeddings_matrix)[1]
  jarak         = rep(10,batas_matriks)
  for(i in 1:batas_matriks){
